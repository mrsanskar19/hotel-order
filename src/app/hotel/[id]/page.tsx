...
export default function Home({ params }: HotelPageProps) {
  ...
  const handleAddToCart = (item: MenuItem, quantity: number = 1, notes?: string, fromSheet: boolean = false) => {
  ...
  async function asyncFuncation(){
    ...
    const cat = await getData(`hotels/${id}/categories`);
    // const menus = await getData(`hotels/${id}/items`)
    if(selectedCategory){
      const items = await getData(`hotels/${id}/categories/${selectedCategory}/items`);
      setItem(items)
    }else{
      // setItem(menus)
    }
  ...
  <h2 className="mb-3 text-lg font-semibold font-headline">Dietary</h2>
  ...
  <div className="flex flex-wrap gap-2">
    ...
    >
                    ...
                    <React.Fragment key={item.id}>
                      <div
                        className={cn(
                          ...
                          item.available ? "hover:shadow-md hover:bg-secondary/40 cursor-pointer" : "opacity-60 cursor-not-allowed"
                        )}
      ...
      <ItemDetailsSheet
        ...
        onAddToCart={(item, quantity, notes) => handleAddToCart(item, quantity, notes, true)}
        onWriteReview={() => {
          setIsItemSheetOpen(false);
